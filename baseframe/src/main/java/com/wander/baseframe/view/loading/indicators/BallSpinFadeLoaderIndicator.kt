package com.wander.baseframe.view.loading.indicators

import android.animation.ValueAnimator
import android.graphics.Canvas
import android.graphics.Paint
import com.wander.baseframe.view.loading.Indicator
import java.util.*
...
class BallSpinFadeLoaderIndicator : Indicator() {
    ...
    override fun draw(canvas: Canvas, paint: Paint) {
        ...
        for (i in 0..7) {
            canvas.save()
            val point =
                circleAt(getWidth(), height, (width / 2 - radius).toFloat(), i * (Math.PI / 4))
